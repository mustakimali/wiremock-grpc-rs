    ...
    ($name:literal, $type:ident) => {
        ...
        impl $type {
            ...
            pub async fn start_default() -> Self {
                let port = $type::find_unused_port()
                    .await
                    .expect("Unable to find an open port");

                $type::new(port).start().await
            }
            ...
            pub fn setup<M>(&mut self, r: M) -> $type
            where
                M: Then + Mountable,
            {
                r.mount(self);
        ...
        impl<B> tonic::codegen::Service<http::Request<B>> for $type
        where
            B: Body + Send + 'static,
